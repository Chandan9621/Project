...
public class UserBookService {
	...
	public JSONArray setBookAndUserName(String token, List<Integer> bookId,Integer id) {
           if (token != null) {
			int uid = 0;
//			List<Integer>li1=bookId;
			String name = getUsernameFromToken(token);
			boolean tokenExpiration = isTokenExpired(token);
			UserEntity ue=ur.findByName(name);
			System.out.println("---------"+ue.getName());
			int userid=ue.getUserid();
			if (ue != null && !tokenExpiration && userid==id) {
				List<UserBookEntity>booklist=ubr.findByUserid(id);
				
				System.out.println("Result3"+booklist);
				for(int k=0;k<booklist.size();k++) {
					ids.add(booklist.get(k).getBookid());
				}
				System.out.println("ChandanPandey"+ids);
				for(int i=0;i<bookId.size();i++) {
					...
					System.out.println("con"+(j==ids.size()));
